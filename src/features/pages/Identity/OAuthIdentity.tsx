...
import { UserCookie } from "../../../app/models/User";
...
export const OAuthIdentity = () => {
    ...
    const handleOAuthRedirect = () => {
        window.open(
            "http://localhost:5110/api/users/external-login",
            "GoogleLogin",
        ...
        window.addEventListener("message", (event) => {
            if (event.data.status === "success") {
                const cookieResponse = Cookies.get('user');
                if(!cookieResponse) return;

                const user : UserCookie = JSON.parse(cookieResponse);
                if (user) {
