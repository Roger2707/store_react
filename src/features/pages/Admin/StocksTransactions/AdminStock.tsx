...
import { SearchProductDetailStock } from "./SearchProductDetailStock";
...
export const AdminStock = () => {
    ...
    useEffect(() => {
        if (stockDTO.productDetailId !== '') {
            ...
            const map = new Map<string, WarehouseProductQuantity>();

            for (const s of stockDTO.stockDetail) {
                if (map.has(s.warehouseId)) {
                    map.get(s.warehouseId)!.quantity += s.quantity;
                } else {
            ...
            const warehousesQuantities = Array.from(map.values());
            ...
            let newWarehouseDropdown = [wareHouseDropdownDefault, ...warehousesQuantities];
